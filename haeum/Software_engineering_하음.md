...
1. iterator pattern
- 내부 구조를 모르더라도 내부 원소에 순차적으로 접근할 수 있게 하기 위하여 인터페이스를 통일할 때 사용한다.
2. observer pattern
- 객체의 상태 변화를 관찰하여 변화가 발생하면 변화와 연관된 객체들에게 알림을 보내어 자동으로 갱신될 수 있게 한다. 
3. state pattern
- 내부 상태가 변할 때 객체의 행동이 달라지도록 프로그램을 작성하는 것으로, 상태에 따라 바뀌는 프로그램을 작성할 때에 유용하다.
4. 화이트박스 테스트란
- 내부 코드를 아는 상태로, 프로젝트에 기반해서 그 루프와 조건등을 테스트하는 것이다. 
5. branch coverage와 condition coverage를 비교하시오
- branch coverage는 모든 조건문에서 true와 false가 한 번씩 나오게 하는 것이다. conditions coverage는 이와 비슷하지만 조건문 안에 있는 더 작은 조건까지 모두 true와 false가 한 번씩은 나오게 설계하는 것이다.
6. MC/DC coverage
- branch coverage와 condition coverage를 모두 만족하는 coverage다. 여러 개의 원자식 중 하나의 원자식만 달라졌는데 전체 결과값이 달라지는 것을 MC/DC라고 하는데, 이들을 모아둔 것이다.
7. 인스펙션에 대해서 설명하세요.
- 코드를 작성하지 않은 사람들이 코드에 문제가 있는지 찾아보는 것으로 코드를 실행하지 않고 테스팅한다.
8. 테스트 드라이버와 테스트 스텁에 대해서.
- 테스트 드라이버는 상향식 설계방법에서 주로 사용하는데, 상위 서버가 미구현 상태일 때, 이를 대체한다. 테스트 스텁은 하향식 설계방법에서 주로 사용하는데, 하위 모듈이 미완성일 때 이 모듈을 대체한다.
9. 테스팅과 디버깅의 차이는?
- 테스팅은 문제의 유무를 확인하고 디버깅은 문제의 위치를 찾아내어 고치는 작업이다. 테스팅 이후에 디버깅을 한다.
10. 블랙박스 테스팅과 화이트 박스 테스팅의 특징과 차이에 대해 설명하세요.
- 블랙박스 테스팅은 내부 구조를 모르는 상태로 명세만 가지고 테스팅을 한다. 화이트박스 테스팅은 반대로 내부 구조만 가지고 그 안의 조건과 루프를 테스팅하여 문제를 찾아낸다.
11. 블랙박스 테스팅의 장점에 대해 설명하세요.
- 블랙박스 테스팅은 코드가 필요 없고, 명세의 논리적 결함을 찾을 수 있다. 따라서 테스트 대상의 규모에 상관없이 테스트 할 수 있다.
12. create/select test case의 3종류에 대해 설명하세요.
- statement coverage : 모든 문장들이 한 번씩은 수행하도록 만든 test suite
- branch coverage : 모든 조건문이 true랑 false 최소 한 번은 수행할 수 있게 만든 test suite
- condition coverage : 조건문 안의 원자식까지 모두 참거짓을 한 번씩 수행할 수 있게 만든 test suite
